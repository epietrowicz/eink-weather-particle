...
bool JsonBuffer::addChunkedData(const char *event, const char *data, size_t chunkSize)
{
	...
	{
		Log.info("Need to allocate more space for data (index: %d) %d + %d > %d", responseIndex, curOffset, len, bufferLen);
		// Need to allocate more space for data
...
JsonParser::JsonParser(char *buffer, size_t bufferLen, JsonParserGeneratorRK::jsmntok_t *tokens, size_t maxTokens) : JsonBuffer(buffer, bufferLen), tokens(tokens), maxTokens(maxTokens)
{
}
